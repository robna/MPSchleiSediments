...
#def get_particles(connection):
MP = pd.read_sql_query(particleQuery, connection)
#    return MP

#MP = get_particles(MPDB_connect())

(MP.pipe(poly_exclude)
 .pipe(particle_amplification)
 .pipe(geom_mean)
 .pipe(shape_colour)
 )

env_MP, IOW_blind_MP, samples_MP, IPF_blank_MP = separate_MPs(MP)
...
for label, row_content in IPF_blank_MP.iterrows():  # loop through the list of IPFblanks, particle by particle
    ...
    # only need the entry of blank_size_geom_mean to be written as a new cloumn in all lines (i.e. particles) that
