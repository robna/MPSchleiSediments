...
featurelist = ['Concentration', 'ConcentrationA500', 'pred_ConcentrationA500', 'ConcentrationB500',
               ...
               'D10 (µm)', 'D50 (µm)', 'D90 (µm)', '(D90 div D10) (µm)', '(D90 - D10) (µm)', '(D75 div D25) (µm)',
               '(D75 - D25) (µm)',
...
def data_load_and_prep():
    ...
    mp_pdd['polymer_type'] = mp_pdd['polymer_type'].map(shortnames).fillna(mp_pdd[
                                                                               'polymer_type'])  # use abbreviations for polymer names but retain original names for polymers not present in shortnames
...
def pdd2sdd(mp_pdd, regions):
    ...
    mp_added_sed_sdd['pred_ConcentrationA500'] = np.exp(
...
def main():
    ...
    if st.checkbox('Calculate GLM'):
        ...
        family = col1.radio('Select distribution family:', families,
                            index=2)  # for neg.binom use CT-alpha-estimator from here: https://web.archive.org/web/20210303054417/https://dius.com.au/2017/08/03/using-statsmodels-glms-to-model-beverage-consumption/
